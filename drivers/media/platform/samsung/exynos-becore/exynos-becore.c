...
#define BECORE_INPUT_SIZE		0x19b1000
#define BECORE_INPUT_IMAGE_OFFSET	0x30000
#define BECORE_GRID_SIZE			0x18000
#define BECORE_OUTPUT_SIZE		0x89c000
#define BECORE_OUTPUT_PLANE2_OFFSET	0x5bcc40
...
static int becore_alloc_diagnostic(struct becore_device *becore)
{
	...
	ret = becore_alloc_dma_buffer(becore, &becore->input,
				      BECORE_INPUT_SIZE, "Bayer input");
	if (ret)
		return ret;
...
static int becore_run_set(void *data, u64 value)
{
	...
	/* Make every coherent program and input write visible before publication. */
	dma_wmb();
