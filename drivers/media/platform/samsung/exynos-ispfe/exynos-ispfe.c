...
/* Large enough for the 1024x768 NV12 geometry experiment below. */
#define ISPFE_LMP_ML0_MAX_SIZE		0x00120000
			     ...
			     dma_addr_t bayer)
{
	...
	for (i = 0; i < prog->num_cmds; i++) {
		...
		ret = ispfe_pdma_apply_ml0_profile(ispfe, cmd, program + at);
		if (ret)
			return ret;
...
static int ispfe_pdma_program_prepare(struct ispfe_device *ispfe)
{
	...
	for (i = 0; i < ispfe->prog->num_inputs; i++) {
		const struct ispfe_pdma_input *input = &ispfe->prog->inputs[i];
