...
BRCMF_FW_CLM_DEF(4383, "brcmfmac4383-pcie");
...
static const struct brcmf_firmware_mapping brcmf_pcie_fwnames[] = {
	...
	BRCMF_FW_ENTRY(BRCM_CC_4383_CHIP_ID, 0xFFFFFFFF, 4383),
...
static int __brcmf_pcie_pm_enter_active(struct brcmf_bus *bus, bool block)
{
	 ...
	 *
	 * The usage reference is taken unconditionally, not gated on
	 * runtime_pm_enabled: that flag flips during setup while ring accesses
	 * are already in flight, so a conditional get here paired with a
	 * conditional put in leave_active can evaluate the condition
	 * differently on the two sides and corrupt the usage count (observed
	 * as "Runtime PM usage count underflow!" at setup time).  get/put on a
	 * device whose runtime PM is still forbidden are safe and free.
	 */
	if (block) {
...
void brcmf_pcie_pm_leave_active(struct brcmf_bus *bus)
{
	 ...
	 * can take the device to D3.  Unconditional to always balance the
	 * unconditional get in enter_active.
	 */
	pm_runtime_put_autosuspend(&devinfo->pdev->dev);
}
...
static const struct pci_device_id brcmf_pcie_devid_table[] = {
	...
	BRCMF_PCIE_DEVICE(BRCM_PCIE_4383_DEVICE_ID, WCC_SEED),
