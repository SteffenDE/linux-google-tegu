...
static int sec_pmic_acpm_probe(struct platform_device *pdev)
{
	 ...
	 * has no interrupt consumer -- neither its regulators nor its read-only
	 * meter need one, sec_irq_init() ignores it, and its combiner interrupt
	 * has no mainline parent -- so allow it to be absent there.
	 */
