...
/*
 * System suspend.  BLK_HSI1 loses power for suspend-to-RAM, so the modem's
 * link has to be down and the controller's clocks gated before genpd runs
 * the domain's power-down handshake -- the vendor kernel reaches the same
 * state from the endpoint side, where the CP parks the link itself and the
 * root complex is powered off behind it.
 *
 ...
 * system suspend can never leave a trained link in front of a domain that
 * is about to go.
 *
 ...
 * runtime park.  The other channel (WiFi, in HSI2) has no switchable
