...
static u32 exynos_bts_int_floor(u32 total_bw, u32 peak_bw)
{
	u64 bus1_freq;
	u64 int_freq;
	u64 freq;
				 ...
				 u32 mif_floor, u32 int_floor)
{
	...
	int_floor = max(int_floor, exynos_bts_int_floor(total_bw, peak_bw));
	...
	bts->active = true;
	dev_dbg(bts->dev, "%u/%u KB/s -> MIF %lu INT %lu Hz\n",
		total_bw, peak_bw, mif_rate, int_rate);
	return 0;
}
...
static int exynos_bts_icc_set(struct icc_node *src, struct icc_node *dst)
{
	...
	u32 peak_bw = 0, mif_floor = 0, int_floor = 0;
	unsigned int i, j;
	int ret;
	...
	for (i = 0; i < bts->num_masters; i++) {
		...
		if (master->data->realtime)
			rt_bw += node->avg_bw;
		peak_bw = max(peak_bw, node->peak_bw);
	...
	ret = exynos_bts_set_active(bts, min_t(u64, total_bw, U32_MAX),
				   min_t(u64, rt_bw, U32_MAX), peak_bw,
				   mif_floor, int_floor);
	if (!ret) {
		bool changed = false;
