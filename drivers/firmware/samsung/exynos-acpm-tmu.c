 ...
 * asks the firmware to read a zone's temperature, program its trip thresholds
 * and enable/clear its interrupts over a dedicated ACPM IPC channel.
 *
 * Backport of upstream commit 4ba23dfc49b4 ("firmware: samsung: acpm: Add TMU
...
/*
 * The firmware writes its reply over the request buffer, so tx and rx alias.
 * Upstream >= 7.2 spells this as acpm_set_xfer(); open-code it here.
 */
static void acpm_tmu_set_xfer(struct acpm_xfer *xfer, union acpm_tmu_msg *msg,
			      unsigned int acpm_chan_id)
{
	xfer->txd = msg->data;
	xfer->rxd = msg->data;
	xfer->txcnt = ARRAY_SIZE(msg->data);
	xfer->rxcnt = ARRAY_SIZE(msg->data);
...
int acpm_tmu_init(struct acpm_handle *handle, unsigned int acpm_chan_id)
{
	...
	acpm_tmu_set_xfer(&xfer, &msg, acpm_chan_id);
